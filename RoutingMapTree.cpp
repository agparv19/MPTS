...
class RoutingMapTree{
    public:
        ...
        Exchange* findPhone(MobilePhone* m){
            ...
            if (!root->rset->myset->IsMember(m->pnum)){
        ...
        ExchangeList* routeCall(MobilePhone* a, MobilePhone* b){
            ...
            if (base_a == NULL){
                cout<<"MobilePhone "<<a->pnum<<" is switched off\n";
                return NULL;
            }

            if (base_b == NULL){
                cout<<"MobilePhone "<<b->pnum<<" is switched off\n";
                return NULL;
            }
        ...
        void printMob(Exchange* e){

            //Checks for all mobile phones, if they are part of exchange e
            //This can be implemented faster, if along with bloom filter, myset class also stores set<int> 
            
            vector<int> mob; 

            for (auto m : mp_map){

                Exchange* temp = findPhone(m.second);

                if (temp == NULL) continue;

                while(temp->ex_id != e->ex_id){
                    temp = temp->parent();
                    if (temp == NULL) break;
                }  

                if (temp!=NULL) mob.push_back(m.first);
            }
